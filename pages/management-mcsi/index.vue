...
export default Vue.extend({
  ...
  methods: {
    ...
    async getPage () {
      try {
        const page = await this.$axios.$get('/api/page-organization-chart-mcsi?populate=*')
        this.page = page.data.attributes
      } catch (error) { }
    },
    async getOrgCharts () {
      try {
        const orgcharts = await this.$axios.$get('/api/organization-chart-mcsis?populate=*&sort[0]=order')
